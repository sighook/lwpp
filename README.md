...
A collection of patches for the vanilla linux kernel,
useful for pentesters and security engineers.

Currently, only the `5.3.x` kernel is supported.
The new branches will be added soon.

## Setup

- Clone the corresponding branch:

  `git clone --depth 1 --branch 5.3 https://github.com/chinarulezzz/linux-wifi-pentest-patches linux-wifi-pentest-patches-5.3`

- Go to vanilla kernel tree and apply these patches:

  `cd /usr/src/linux3.5; for p in /path/to/linux-wifi-pentest-patches; do patch -p1 < $p; done`
...
- `0006-Override-regdomain-hardcoded-in-EEPROM-with-custom-v.patch`
  ...
  `sudo rmmode carl9170 ; sudo modprobe carl9170 override_eeprom_regdomain=CODE`
